...
#define RP_WFP_DECISION_FLAG_DNS_AUTO            (1u << 0)
#define RP_WFP_DECISION_FLAG_DNS_BOOTSTRAP_PROXY (1u << 1)
...
#define RP_WFP_IOCTL_SET_PROXY_READY CTL_CODE(RP_WFP_DEVICE_TYPE, 0x808, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
typedef struct _RP_WFP_PROXY_READY {
    UINT32 AbiVersion;
    UINT32 Size;
    UINT32 Ready;
    UINT32 Reserved;
...
C_ASSERT(sizeof(RP_WFP_PROXY_READY) == 16);
