...
func (s *WailsService) RunRemoteDesktopNVCodecQualification() (string, error) {
	if s == nil || s.owner == nil || s.owner.bridge == nil {
		return `{"passed":false,"error":"GUI unavailable"}`, nil
	}
	report, _ := s.owner.bridge.RunRemoteDesktopNVCodecQualification()
	data, err := json.Marshal(report)
	if err != nil {
		return `{"passed":false,"error":"failed to encode NVCodec qualification report"}`, nil
	}
	return string(data), nil
}

func (s *WailsService) RunRemoteDesktopNVCodecStressQualification() (string, error) {
	if s == nil || s.owner == nil || s.owner.bridge == nil {
		return `{"passed":false,"error":"GUI unavailable"}`, nil
	}
	report, _ := s.owner.bridge.RunRemoteDesktopNVCodecStressQualification()
	data, err := json.Marshal(report)
	if err != nil {
		return `{"passed":false,"error":"failed to encode NVCodec stress qualification report"}`, nil
	}
	return string(data), nil
}
...
func (s *WailsService) SetRemoteDesktopDisplay(displayID string) (string, error) {
	if s == nil || s.owner == nil || s.owner.bridge == nil {
		return `{"ok":false,"message":"GUI unavailable"}`, nil
	}
	if err := s.owner.bridge.SetRemoteDesktopDisplay(displayID); err != nil {
		data, _ := json.Marshal(map[string]any{"ok": false, "message": err.Error()})
		return string(data), nil
	}
	return `{"ok":true}`, nil
