...
func webConfigJSON(b *bridge.UIBridge) string {
	...
	payload := struct {
		...
		Network:             map[string]any{"mode": cfg.Network.Mode, "dns_mode": cfg.Network.DNSMode, "exclude_processes": cfg.Network.ExcludeProcesses},
		...
		Runtime: map[string]any{"serverAddress": state.Runtime.Server.Address, "quicPort": state.Runtime.Server.QUICPort,
			...
			"transport": state.Runtime.Transport.Mode, "networkMode": state.Runtime.Network.Mode, "dnsMode": state.Runtime.Network.DNSMode,
