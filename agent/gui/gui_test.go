...
func TestRemoteDesktopFollowViewport(t *testing.T) {
	...
	for _, want := range []string{
		...
		"options.codec === 'h265'",
		"!remoteDesktopTargetSupportsChroma(targetID, 'h265', options.chroma)",
		"目标未上报 H.265 / HEVC ' + requestedChroma + ' 编码能力",
...
func TestRemoteDesktopRuntimeDisplaySwitcher(t *testing.T) {
	...
	for _, want := range []string{
		...
		"等待新媒体 Generation",
		"releaseDesktopInput()",
		"status.displaysReady",
		"Array.isArray(status.displays) ? status.displays.slice() : []",
		"Host 会自动推送实时显示器列表",
