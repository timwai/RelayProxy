...
func TestDesktopAuthorizationDoesNotRequireRDPService(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO users
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		"user-1", "desktop-owner", "hash", "Desktop Owner", "admin", "active", now, now); err != nil { t.Fatal(err) }
	insertDevice := func(id, name, fingerprint, installation, caps string) {
		...
		if _, err := db.Exec(`INSERT INTO devices
			...
			EnrollmentApproved, caps, caps, now, now, now); err != nil { t.Fatal(err) }
	}
	...
	if _, err := db.Exec(`INSERT INTO rdp_access_grants
		...
		VALUES (?, ?, ?, ?)`, "controller", "home-target", "user-1", now); err != nil { t.Fatal(err) }
	allowed, err := db.AuthorizeDesktop("controller", "home-target")
	if err != nil { t.Fatal(err) }
	if !allowed { t.Fatal("desktop authorization was denied without an RDP service") }
	rdpAllowed, err := db.AuthorizeRDP("controller", "home-target")
	if err != nil { t.Fatal(err) }
	if rdpAllowed { t.Fatal("native RDP authorization unexpectedly succeeded") }
	targets, err := db.ListRemoteDesktopTargetsForController("controller")
	if err != nil { t.Fatal(err) }
	if len(targets) != 1 { t.Fatalf("targets=%d want 1", len(targets)) }
	target := targets[0]
